  ...
  <div class="page2">
    <ul class="text">
      <li>
        <span>
          电流计类型：
          <i>{{titleData.deviceType}}</i>
        </span>
        <span>
          电流计状态：
          <i>{{titleData.isOnline == 1 ? '在线' : '离线'}}</i>
        </span>
        <span>
          资产当前电流
          <i>{{titleData.electricValue+'/A'}}</i>
        </span>
        <span>
          总开机时长(分)：
          <i>{{titleData.totalStartTime}}</i>
        </span>
        <span>
          昨日开机时长(分)：
          <i>{{titleData.yesterdayStartTime}}</i>
        </span>
      </li>
      <li>
        <span>
          近七天是否运行：
          <i>{{titleData.isWeekStart == 1 ? '是' : '否'}}</i>
        </span>
        <span>
          昨日非常规时间是否运行：
          <i>{{titleData.isExceptStart == 1 ? '是' : '否'}}</i>
        </span>
      </li>
    </ul>
    <div class="chart-box">
      <div class="history item">
        ...
        <div class="chart" id="rundata_history"></div>
      </div>
      <div class="start item">
        ...
        <div class="chart" id="start_time"></div>
      </div>
    </div>
  </div>
</template>
            ...
            this.$http.get('/api/data/electricData', data).then(d => {
              ...
              (hour > 9 ? hour : '0' + hour)
                ...
                const line_box = this.$echarts.init(
                    document.getElementById('rundata_history')
                );
